#!/usr/bin/env python3
"""
Simple Database Demo - The Ship's Foundation

A simplified demonstration of the database system functionality
that avoids session management issues.
"""
...
def main():
    ...
    print("🗄️ Simple Database Demo - The Ship's Foundation")
    print("=" * 60)
    ...
    # Get a specific contract to update
    api_contract = db_manager.get_contract("api.02")
    if api_contract:
        print(f"✅ Found contract: {api_contract['id']} - {api_contract['title']}")
        
        # Update the contract
        updates = {
            "progress_percentage": 75,
            "worker_notes": "Database system implementation in progress"
        }
        
        success = db_manager.update_contract("api.02", updates, actor="demo_script")
        if success:
            print(f"✅ Updated contract api.02")
        else:
            print(f"❌ Failed to update contract api.02")
    
    print("\n5. 📝 Contract Logs")
    print("-" * 40)
    
    # Get contract logs
    logs = db_manager.get_contract_logs("api.02", limit=5)
    print(f"✅ Retrieved {len(logs)} log entries for api.02")
    ...
    if 'error' not in stats:
        ...
        print(f"  Database URL: {conn_info.get('database_url', 'N/A')}")
        print(f"  Pool Size: {conn_info.get('pool_size', 0)}")
    else:
